...
function buttonClick(e){
  ...
  // //if (e.target.closest('#buttons')) {a} else {b}
  
  // if(t.parentNode.id=="buttons"){
  //   document.getElementById("container").appendChild(e.target);
  // }
  // else if(t.parentNode.id=="container"){
  //   document.getElementById("buttons").appendChild(e.target);
  // }
  
  document.getElementById(e.target.closest('#buttons') ? "container" : "buttons").appendChild(e.target.closest('button'));
}
...
async function submit(){
  ...
  if(id){
    d= await loadData();
    if(Number(d.substring(3,4))<checkAns()){
      d=d.substring(0,3)+checkAns()+d.substring(4);
      saveData(d);
    }
  } else{
